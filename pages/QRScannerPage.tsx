      ...
      <div className="flex-1 flex flex-col items-center justify-center px-4 pt-4 pb-6">
        {status === 'idle' && (
          <div className="text-center">
            <div className="w-24 h-24 rounded-xl bg-card/35 flex items-center justify-center mx-auto mb-6">
        ...
        {status === 'scanning' && (
          <div className="w-full max-w-sm flex-1 flex flex-col">
            <div className="relative flex-1 min-h-[320px] rounded-xl overflow-hidden bg-black">
              <div id={containerId} className="absolute inset-0" />
              <div className="absolute inset-0 pointer-events-none">
                <div className="absolute inset-0 bg-black/20" />
                <div className="absolute left-1/2 top-1/2 w-[260px] h-[260px] -translate-x-1/2 -translate-y-1/2 rounded-xl border border-neon/35" />
              </div>
            </div>
            <div className="mt-auto w-full">
              <button
                onClick={() => { Haptic.tap(); stopScanning(); }}
                className="w-full py-3 bg-card/35 text-white font-semibold rounded-xl flex items-center justify-center gap-2"
              >
                <X size={20} />
                {t('qr_stop')}
              </button>
        ...
        {status === 'success' && (
          <div className="w-full max-w-sm flex-1 flex flex-col">
            <div className="text-center">
              ...
              <div className="bg-card/25 rounded-xl p-3 mb-4 break-all text-left text-xs text-neutral-300 font-mono max-h-24 overflow-y-auto">
                {lastResult}
              </div>
            </div>
            <div className="mt-auto w-full">
